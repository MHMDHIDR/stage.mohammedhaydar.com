...
const NavMenu: React.FC<NavMenuPros> = ({
  ...
  return (
    <div
      className={`relative mx-auto m-2 py-1.5 px-2 text-sm leading-relaxed transition-colors duration-150 bg-white dark:bg-gray-700 border border-gray-400 rounded-lg dropdown:block focus:outline-none hover:border-gray-600 focus:shadow-outline focus:border-gray-900 cursor-pointer${
        isOptions ? ' max-w-fit' : ''
      }`}
      role='menuitem'
      aria-haspopup='true'
    ...
    >
      ...
      <ul
        className={`absolute flex flex-col gap-1.5 left-1/2 -translate-x-1/2 min-w-max p-2 mt-4 text-sm bg-white dark:bg-gray-700 border border-gray-100 dark:border-gray-700 z-[1000] rounded-lg shadow-l${
          !isOpen ? ' hidden' : ''
        }${className ? ' ' + className : ''}`}
        aria-label='submenu'
      >
